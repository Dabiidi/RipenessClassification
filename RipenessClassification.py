...
def color_brown(img):   
        ...
        brown_lower = np.array([0, 10, 50], dtype=np.uint8)
        brown_upper = np.array([20, 150,200], dtype=np.uint8)
...
def color_green(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    green_lower =np.array([23, 93, 96],dtype=np.uint8)
...
def color_yellow(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    yellow_lower = np.array([0, 155, 92],dtype=np.uint8)
    yellow_upper = np.array([177,227,255],dtype=np.uint8)
...
def merge_picture():
            ...
            img_02 = Image.open("/home/pi/images/2.jpg")
            img_03 = Image.open("/home/pi/images/3.jpg")
            img_04 = Image.open("/home/pi/images/4.jpg")
